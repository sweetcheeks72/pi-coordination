...
export function registerWorkerTools(pi: ExtensionAPI): void {
	...
	// Wrap deprecated tools with warnings
	const deprecatedMappings: Record<string, string> = {
		send_message: "agent_chat({ to: '...', content: '...' })",
		check_messages: "agent_chat({ action: 'inbox' })",
		share_discovery: "agent_chat({ to: 'all', topic: '...', content: '...' })",
		escalate_to_user: "agent_chat({ to: 'user', question: '...', options: [...] })",
		signal_contract_complete: "agent_sync({ action: 'provide', item: '...' })",
		wait_for_contract: "agent_sync({ action: 'need', item: '...' })",
		complete_task: "agent_work({ action: 'complete', result: '...' })",
		update_step: "agent_work({ action: 'step', step: N })",
		add_discovered_task: "agent_work({ action: 'add', description: '...' })",
		report_deviation: "agent_work({ action: 'deviation', description: '...' })",
		read_plan: "agent_work({ action: 'plan' })",
		reserve_files: "file_reservations({ action: 'acquire', patterns: [...] })",
		release_files: "file_reservations({ action: 'release', patterns: [...] })",
	};

	for (const tool of tools) {
		const deprecatedTo = deprecatedMappings[tool.name];
		if (deprecatedTo) {
			const originalExecute = tool.execute;
			tool.execute = async function(toolCallId, params, onUpdate, ctx) {
				console.warn(`[DEPRECATED] ${tool.name} → ${deprecatedTo}`);
				return originalExecute.call(this, toolCallId, params, onUpdate, ctx);
			};
		}
